...
nameDialog:boolean;
  constructor(private FS: FilterService ,private fb: FormBuilder,private router: Router ) { }
  ...
  getFilter(){
    return this.FS.getFilter(parseInt(localStorage.getItem('projectCode'))).subscribe((result)=> {
      debugger;
      result.forEach(element => {
        ...
        switch(fieldFilter.FieldType){
          ...
          case "date":
          case "datetime":
            fieldFilter.Filter=element.dateValue;
          break;
          case ("bit"):
  ...
  projectName(){
    debugger;
    return this.FS.projectName(this.companycode)
   .subscribe((result)=> {
   ...
   getTablesName(){
     debugger;
    return this.FS.getTablesName(this.companycode,this.filterForm.controls.projectName.value.projectname )
    .subscribe((result)=> {
      debugger;
   ...
   getColumnName(){
    debugger;
    return this.FS.getColumnName(this.projectCode,this.filterForm.controls.tableOfFilter.value.Name)
    .subscribe((result)=> {
  ...
  getFilterTypes(){
    ...
    return this.FS.getFilterTypes(this.projectCode,filForm.tableOfFilter.value.Name,filForm.columsName.value.Name)
    .subscribe((result)=> {   
      debugger;
  ...
  getFilterInput(){
    ...
    if(this.typeOfFielde=="number"){
      if(this.selectedFilter=="טווח ערכים")
  ...
  filter(){
   ...
   {
  this.filterToSend= this.arrayOfSelectedFilter.filter(val => this.selectedFilters.includes(val));
...
saveFilter()
{
  ...
  {
    fieldFilter.Filter=this.filterForm.controls.valueOfFilter.value+ "-"+this.filterForm.controls.number2.value;
  }
  else
  fieldFilter.Filter=this.filterForm.controls.valueOfFilter.value;
  debugger;
  filter.FilterList.push(fieldFilter);
...
if(resulte)
this.arrayOfSelectedFilter = this.arrayOfSelectedFilter.filter(val => val.id !== filter.id);
  });
};
...
return this.FS.getColumnName(this.projectCode,this.newFieldFilter.TableName)
    .subscribe((result)=> {
      this.columNameFilter=result;
      this.columNameForFilter=new Array<Colum>();
      for (let index = 0; index < this.columNameFilter.length; index++) {
...
saveFieldFilter(){
  ...
  {
    field.Filter=this.filterForm.controls.valueOfFilter.value+ "-"+this.filterForm.controls.number2.value;
  }
  else
  field.Filter=this.filterForm.controls.valueOfFilter.value;
  let index=this.arrayOfSelectedFilter.map(function(e){return e.id;}).indexOf(this.newFieldFilter.id);
  this.arrayOfSelectedFilter[index].FilterList.push(field);
  this.FS.saveFilter(this.arrayOfSelectedFilter[index]).subscribe((result)=> {
    let a:Number;
    a=result;
  });
  this.filterDialog=false;
