...
delete(route: string, parameter: any = null): Observable<any> {
  debugger;
  this.httpOptions = {
    headers: new HttpHeaders({
      'Content-Type': 'application/json'
  ...
  if (parameter instanceof (Object)) {
    return this.httpClientService.delete(this.userControllerUrl + route, parameter);
  }
  return this.httpClientService.delete(this.userControllerUrl + route,this.httpOptions);
}
